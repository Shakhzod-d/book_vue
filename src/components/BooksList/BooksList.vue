...
<template>
  <div class="grid grid-cols-6 gap-10">
    <div
      class="w-full bg-white shadow-xl min-h-[330px] p-4 rounded-lg cursor-pointer hover:translate-y-[-6px] transition-all hover:shadow-2xl"
      v-for="item of books"
      :key="item"
    >
      ...
      <p class="font-semibold mb-1">
        Nashr qilingan yili: <b>{{ item.nashr_qilingan_yili }}</b>
      </p>
      <p class="font-medium mb-1">
        Nashriyot: <b>{{ item.nashriyot }}</b>
      </p>
      <p class="font-medium mb-1">
        Til: <b>{{ item.til }}</b>
      </p>
